...
import static com.github.bakuplayz.cropclick.common.Languages.Menu.WORLDS_ITEM_GROWTH_TIPS;
import static com.github.bakuplayz.cropclick.common.Languages.Menu.WORLDS_ITEM_STATUS;

/**
 * A class representing the Worlds menu scoped for OfflineGrowth.
 *
 * @author BakuPlayz
 * @version 2.2.0
 * @since 2.2.0
...
public final class WorldsMenu extends AbstractWorldsMenu {
    ...
    private static List<String> getItemLore(@NotNull CropClick plugin, @NotNull FarmWorld world) {
        String status = WORLDS_ITEM_STATUS.builder(plugin)
                                .replace("%status%", world.isAddonBanished(getAddon(plugin, OfflineGrowthAddon.NAME)))
                                .build();
        return WORLDS_ITEM_GROWTH_TIPS.builder(plugin).append(status).buildAsList();
    }
