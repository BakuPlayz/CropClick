...
import static com.github.bakuplayz.cropclick.common.Languages.Menu.WORLDS_ITEM_TOWNY_TIPS;
...
public final class WorldsMenu extends AbstractWorldsMenu {
    ...
    private static List<String> getItemLore(@NotNull CropClick plugin, @NotNull FarmWorld world) {
        String status = WORLDS_ITEM_STATUS.builder(plugin)
                                .replace("%status%", world.isAddonBanished(getAddon(plugin, TownyAddon.NAME)))
                                .build();
        return WORLDS_ITEM_TOWNY_TIPS.builder(plugin).append(status).buildAsList();
    }
