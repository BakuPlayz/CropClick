...
@AllArgsConstructor
public final class MySQLProvider implements QueryProvider {
    ...
    public <T> InsertQuery<T> insert(@NotNull String table, @NotNull Class<T> clazz, boolean ifNotExist) {
        return new com.github.bakuplayz.cropclick.database.query.mysql.InsertQuery<>(table, clazz, ifNotExist);
    }
    ...
    public <T> SelectQuery<T> select(@NotNull String table, @NotNull Class<T> clazz) {
        return new com.github.bakuplayz.cropclick.database.query.mysql.SelectQuery<>(jsonMapper, table, clazz);
    }
