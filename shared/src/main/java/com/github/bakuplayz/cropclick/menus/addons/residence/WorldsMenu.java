...
import static com.github.bakuplayz.cropclick.common.Languages.Menu.WORLDS_ITEM_RESIDENCE_TIPS;
import static com.github.bakuplayz.cropclick.common.Languages.Menu.WORLDS_ITEM_STATUS;

/**
 * A class representing the Worlds menu scoped for Residence.
 *
 * @author BakuPlayz
 * @version 2.2.0
 * @since 2.2.0
...
public final class WorldsMenu extends AbstractWorldsMenu {
    ...
    private static List<String> getItemLore(@NotNull CropClick plugin, @NotNull FarmWorld world) {
        boolean isBanished = world.isAddonBanished(getAddon(plugin, ResidenceAddon.NAME));
        String status = WORLDS_ITEM_STATUS.builder(plugin)
                                .replace("%status%", isBanished)
                                .build();
        return WORLDS_ITEM_RESIDENCE_TIPS.builder(plugin).append(status).buildAsList();
    }
