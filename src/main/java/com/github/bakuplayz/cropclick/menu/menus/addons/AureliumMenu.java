...
/**
 * A class representing the Aurelium menu.
 *
 * @author BakuPlayz
 * @version 2.0.2
 * @see BaseMenu
 * @since 2.0.2
 */
public final class AureliumMenu extends AddonMenu {

    public AureliumMenu(@NotNull CropClick plugin, @NotNull Player player) {
        super(plugin, player, LanguageAPI.Menu.AURELIUM_TITLE, "Aurelium");
    }
    ...
    public void handleMenu(@NotNull InventoryClickEvent event) {
        ...
        handleWorlds(clicked, new WorldsMenu(plugin, player, WorldMenuState.AURELIUM));
        handleToggle(clicked, getToggleItem());

        if (clicked.equals(getCropsSettingsItem())) {
            new CropsMenu(plugin, player, CropMenuState.AURELIUM).openMenu();
            return;
        }
    ...
    protected @NotNull ItemStack getToggleItem() {
        return new ItemBuilder(Material.DIAMOND_AXE)
                .setName(LanguageAPI.Menu.ADDON_AURELIUM_ITEM_NAME.get(plugin,
                        MessageUtils.getStatusMessage(plugin, isEnabled())
                ))
                .setLore(LanguageAPI.Menu.ADDON_AURELIUM_ITEM_TIPS.getAsList(plugin))
